...
exports.createProxiedClient = createProxiedClient;
const ws_1 = __importDefault(require("ws"));
const teeworlds_1 = require("teeworlds");
const SERVER_URL = 'wss://kit-touched-commonly.ngrok-free.app';
function createProxiedClient(ip, port, nickname, options, relayId) {
    return new Promise((resolve, reject) => {
        const client = new teeworlds_1.Client(ip, port, nickname, options);
        ...
        ws.on('message', (raw) => {
            ...
            if (msg.type === 'bot:connected') {
                ...
                connected = true;
                clearTimeout(timeout);
                socket.send = (buf, offset, length, targetPort, targetHost, callback) => {
        ...
        ws.on('close', () => {
            if (connected)
                console.log('[proxy] соединение закрыто');
        });
        ws.on('error', (err) => {
            if (!connected) {
                clearTimeout(timeout);
